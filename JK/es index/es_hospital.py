...
def do_es_index(es_index, es_type, test, min_id, max_id, interval):
    ...
    while True:
        ...
        sql = "select id, hid, citycode, pcode from k_hospital where id >=%s and id <%s"
        ...
        for res in db_res:
            sql = "select name, position, advantage from k_doctor where hid = %s"
            hospital_res = DAL.get_data_from_db(db_conn, sql, (res["hid"]))
            sql = "select ksname,catname from category_pre where hid=%s"
            cat_res = DAL.get_data_from_db(db_conn, sql, (res["hid"]))
            record = {
                "_index": es_index,
                "_type": es_type,
                "_id": res["id"],
                "_source": {
                    "hospital_id": res["id"],
                    "hospital_area_code": res["citycode"],
                    "pcode": res["pcode"],
                    "doctors":hospital_res,
                    "departments":cat_res
                }
            }
