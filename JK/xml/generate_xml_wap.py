...
def gen_xml(es, es_index, ask_set, pid):
    ...
    for _id in xrange(1, 1333699):
        ...
        if len(res["answer"]) > 0:
            if len(res["answer"]) > 0:
                ...
                answer = best_answer["answer_bqfx"] + best_answer["answer_zdyj"] + best_answer["answer_summary"] + \
        ...
        record={
            "key":re.sub(u'[\u3000,\xa0,\x08,\x3000,\x20]', u'', the_key.strip().replace("\r\n","").replace("\n","").replace("\t","")),
            "title":"%s_169健康网" % (re.sub('\s','',re.sub(u'[\u3000,\xa0,\x08,\x3000,\x20]', u'',title.strip().replace("\r\n","").replace("\n","").replace("\t","")))),
            "question":re.sub('\s','',re.sub(u'[\u3000,\xa0,\x08,\x3000,\x20]', u'',the_question.strip().replace("\r\n","").replace("\n","").replace("\t",""))),
            "count":count,
            "time":"%s年%s月%s日" % (date.year, date.month, date.day),
            "url":"http://m.169kang.com/question/%s.html?from=sgoaw&z=0%s_%s" % (title_id, sequence, count),
            "showurl":"169健康网 - m.169kang.com",
            "answer": re.sub('\s','',re.sub(u'[\u3000,\xa0,\x08,\x3000,\x20]', u'', answer.strip().replace("\r\n","").replace("\n","").replace("\t","")))
        }
        ...
        if len(items) == 2000:
            fp = codecs.open("./jk_wap_%s_%s.xml" % (pid, times), "w", "utf-8")
    ...
    if len(items) > 0:
        fp = codecs.open("./jk_wap_%s_last.xml" % (pid), "w", "utf-8")
        fp.write("%s" % (xml + "".join(items) + "</DOCUMENT>"))
        fp.close()



def do_xml_wap(test, es_index, pid):
    ...
    gen_xml(es, es_index, ask_set, pid)
...
if __name__ == "__main__":
    ...
    do_xml_wap(args.test, args.es_index, int(args.ID))
