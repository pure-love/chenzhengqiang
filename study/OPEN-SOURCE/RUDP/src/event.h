...
/*----------------------------------------------------------------------------
  File:   event.h
  Description: RUDP event management: Input event handling, registering file descriptors, 
...
int event_timeout(struct timeval timer,  
		       int (*callback)(int, void*), void *callback_arg, const char *idstr);
...
int event_fd(int fd, int (*callback)(int, void*), void *callback_arg, const char *idstr);
int eventloop();
