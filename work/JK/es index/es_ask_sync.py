...
def do_es_index(test, es_index, es_type, min_id, max_id, bulks):
    ...
    times = 0
    es = get_es_handler(CONFIG[test]["ELASTICSEARCH_HOST"], CONFIG[test]["ELASTICSEARCH_PORT"])
    search_es = get_es_handler(CONFIG["OFFICIAL"]["ELASTICSEARCH_HOST"], CONFIG[test]["ELASTICSEARCH_PORT"])
    query = {"query": {"match_all": {}}}
    scanResp = helpers.scan(client=es, query=query, scroll="10m", index="searchindex", timeout="10m")
    count = 0
    for resp in scanResp:
        ID = get_id_from_url(resp["_source"]["url"])
        if ID == -1:
            continue
        ...
        record = {
                    ...
                    "answer_accept":resp["_source"]["is_took"],
                    "date":resp["_source"]["inputtime"]
                     }
                 }
        records.append(record)
        count +=1
        if count == bulks:
            helpers.bulk(search_es, records)
            times +=1
            count = 0
...
if __name__ == "__main__":
    ...
    do_es_index(args.test.upper(), args.index, args.doc_type,
                int(args.min_id), int(args.max_id), int(args.bulks))
