...
test_query={
 "query":
  ...
  "_source":["id","title","description","sex","ops_price","update_time", "img_urls", "url"]
}
...
def do_es_index(es_index, es_type, test, min_id, max_id, interval):
    ...
    while True:
        ...
        sql = "select id, title, description, sex,url,updatetime, shoushu_price as ops_price from k_anli where id >=%s and id <%s"
        ...
        for res in db_res:
            sql = "select img_urls from k_anli_data where id = %s"
            img_res = DAL.get_data_from_db(db_conn, sql, (res["id"]))
            ...
            record = {
                ...
                "_source": {
                    ...
                    "ops_price":res["ops_price"],
                    "img_urls":json.dumps(img_res[0]) 
                }
            }
