...
void handle_cmd_options( int ARGC, char * * ARGV, struct CMD_OPTIONS & cmd_options )
{
    ...
    {
         cmd_options.config_file="/etc/services1986.conf";
    }    
}
...
void read_config( const char * config_file, CONFIG & config )
{
    ...
    while(getline(ifile,line))
    {
         ...
         while( line_iter != line.end() )
         {
              ...
              else
              {
                   ...
                   else if(key.compare("event-loop") == 0)
                   {
                       config.ev_loop = atoi(value.c_str());
                   }
    ...
    if( config.log_file.empty()   || config.IP.empty() || config.PORT == -1 || 
        config.log_level == -1   || config.run_as_daemon == -1 )
    {
...
#@desc:
*/
int  read_http_header(int socket_fd, void *buffer, size_t buffer_size )
{
    ...
    while(  true )
    {
         ...
         if( now - prev >= 20 )
         return -1;   
         if( (received_bytes = read(socket_fd,buffer_forward,sizeof(char))) <= 0 )
         {
         ...
         if( !read_http_header_done && *buffer_forward == '\r' )
         {
              ...
              while( true )
              {
                   if((received_bytes = read( socket_fd,buffer_forward,left_bytes)) <= 0)
                   {
                        JUDGE_RETURN(received_bytes);
                   }
                   read_bytes+=received_bytes;
...
int read_specify_size2( int fd, void *buffer, size_t total_bytes, bool & timeout )
{
    size_t  left_bytes;
    int received_bytes;
    uint8_t *buffer_forward;
