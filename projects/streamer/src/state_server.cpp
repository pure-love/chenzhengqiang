...
void * state_server_entry( void * args )
{
    ...
    struct ev_loop *state_server_loop = ev_loop_new(EVBACKEND_EPOLL);
    if( state_server_loop == NULL )
    {
        log_module(LOG_INFO,"STATE_SERVER","EV_LOOP_NEW FAILED:%s",LOG_LOCATION);
        return NULL;
    }
    ...
    if( listen_watcher == NULL )
    {
        ...
        log_module(LOG_INFO,"STATE_SERVER","ALLOCATE MEMORY FAILED:%s",LOG_LOCATION);
        return NULL;
    }
...
void accept_request_cb(struct ev_loop * state_server_loop, struct ev_io *listen_watcher, int revents )
{
    ...
    if( EV_ERROR & revents )
    {
        log_module(LOG_INFO,"STATE SERVER","LIBEV ERROR FOR EV_ERROR:%d--%s",EV_ERROR,LOG_LOCATION);
        return;
    }
    client_fd = accept(listen_watcher->fd, (struct sockaddr *)&client_addr, &len );
    if( client_fd < 0 )
    {
        log_module(LOG_INFO,"STATE SERVER","ACCEPT ERROR:%s--%s",strerror(errno),LOG_LOCATION);
        return;
    }
    ...
    {
         close(client_fd);
         log_module(LOG_INFO,"STATE SERVER","ALLOCATE MEMORY FAILED:%s",LOG_LOCATION);
         return;
    }
    ...
    sdk_set_sndbuf( client_fd, 65535); 
...
void receive_cb(struct ev_loop *state_server_loop, struct ev_io * receive_request_watcher, int revents )
{
    ...
    if( EV_ERROR & revents )
    {
        log_module(LOG_INFO,"STATE SERVER","RECEIVE_CB:ERROR FOR EV_ERROR:%s",LOG_LOCATION);
    ...
    {     
          ...
          {
              log_module(LOG_INFO,"STATE SERVER","RECEIVE_CB ERROR--READ_HTTP_HEADER:BUFFER SIZE IS TOO SMALL");
          }
          close(receive_request_watcher->fd);
          DO_STATE_SERVER_RECEIVE_CLEAN();
    }
    http_request[received_bytes]='\0';
    log_module(LOG_INFO,"STATE SERVER","RECEIVE_CB--HTTP REQUEST:%s",http_request);
    //parse_http_request(request, req_info);
    std::string path="";
    std::string command="";
    char *request = http_request;
    ...
    shutdown(receive_request_watcher->fd,SHUT_RD);
    DO_STATE_SERVER_RECEIVE_CLEAN();
}
...
void send_system_info_cb(struct ev_loop *state_server_loop, struct ev_io * send_system_info_watcher, int revents )
{
    ...
    if( cpu_occupy < 0 || mem_occupy < 0 ||!ok )
    {
         ...
         write_specify_size(send_system_info_watcher->fd,http_200_ok,strlen(http_200_ok));
         close( send_system_info_watcher->fd );
         ev_io_stop(state_server_loop,send_system_info_watcher);
         delete send_system_info_watcher;
         send_system_info_watcher = NULL;
    ...
    const char * http_200_ok = "HTTP/1.1 200 OK\r\nContent-type:application/json\r\n\r\n";
    write_specify_size(send_system_info_watcher->fd,http_200_ok,strlen(http_200_ok));
    write_specify_size(send_system_info_watcher->fd,serialized_string,strlen(serialized_string));
...
bool startup_state_server( ssize_t listen_fd )
{
    ...
    if( listen_fd < 0 )
    {
        log_module(LOG_INFO,"STATE SERVER","STARTUP_STATE_SERVER","REGISTER_STATE_SERVER:FAILED");
        return false;
    }
