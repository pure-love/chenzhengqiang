                                   ...
                                   AVCodecParserContext *pc, AVPacket *pkt)
{
    ...
    *pden = 0;
    ...
    case CODEC_TYPE_VIDEO:
        if(st->time_base.num*1000L > st->time_base.den){
            *pnum = st->time_base.num;
            *pden = st->time_base.den;
        }else if(st->codec->time_base.num*1000L > st->codec->time_base.den){
...
static int64_t lsb2full(int64_t lsb, int64_t last_ts, int lsb_bits){
    int64_t mask = lsb_bits < 64 ? (((int64_t)1)<<lsb_bits)-1 : ((int64_t)-1);
...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;) {
		...
		//printf("DBD in av_read_frame_internal\n");
        st = s->cur_st;
        if (st) {
 ...
 */
void get_first_packet(AVFormatContext *s, AVPacketList **pktList)
{
	*pktList = s->packet_buffer;
}
...
int av_seek_frame_binary(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    if(st->index_entries){
        ...
        if(e->timestamp <= target_ts || e->pos == e->min_distance){
            ...
            ts_min= e->timestamp;
#ifdef DEBUG_SEEK
        av_log(s, AV_LOG_DEBUG, "using cached pos_min=0x%lld dts_min=%lld\n",
               pos_min,ts_min);
#endif
        }else{
            assert(index==0);
...
#ifdef DEBUG_SEEK
        av_log(s, AV_LOG_DEBUG, "using cached pos_max=0x%lld pos_limit=0x%lld dts_max=%lld\n",
    ...
    while (pos_min < pos_limit) {
#ifdef DEBUG_SEEK
        av_log(s, AV_LOG_DEBUG, "pos_min=0x%lld pos_max=0x%lld dts_min=%lld dts_max=%lld\n",
               pos_min, pos_max,
               ts_min, ts_max);
#endif
        ...
        if(pos == pos_max)
            ...
            no_change=0;
#ifdef DEBUG_SEEK
av_log(s, AV_LOG_DEBUG, "%lld %lld %lld / %lld %lld %lld target:%lld limit:%lld start:%lld noc:%d\n", pos_min, pos, pos_max, ts_min, ts, ts_max, target_ts, pos_limit, start_pos, no_change);
#endif
    ...
    av_log(s, AV_LOG_DEBUG, "pos=0x%lld %lld<=%lld<=%lld\n",
...
static void av_estimate_timings(AVFormatContext *ic)
{
    ...
    if (( 0 /*ic->iformat == &mpegps_demux || ic->iformat == &mpegts_demux*/) && file_size && !ic->pb->is_streamed) {
...
#endif
            if(duration_count[i] && st->codec->time_base.num*101L <= st->codec->time_base.den &&
               st->time_base.num*duration_sum[i]/duration_count[i]*101L > st->time_base.den){
            ...
            if ((st->codec->codec_id == CODEC_ID_MPEG1VIDEO ||
                ...
                if (st->codec_info_nb_frames >= 20) {
                    ...
                    if (coded_frame_rate >= 24.97 &&
                        ...
                        st->r_frame_rate = AV_TIME_BASE_Q;
                    }
                }
            }
...
//        pkt->pts= st->cur_dts;
    ...
    if(st->cur_dts && st->cur_dts != AV_NOPTS_VALUE && st->cur_dts >= pkt->dts){
        av_log(NULL, AV_LOG_ERROR, "error, non monotone timestamps %lld >= %lld\n", st->cur_dts, pkt->dts);
        return -1;
    }
    if(pkt->dts != AV_NOPTS_VALUE && pkt->pts != AV_NOPTS_VALUE && pkt->pts < pkt->dts){
...
static void truncate_ts(AVStream *st, AVPacket *pkt){
    int64_t pts_mask = (2L << (st->pts_wrap_bits-1)) - 1;
                 ...
                 int is_output)
{
	...
	av_log(NULL, AV_LOG_INFO, "%s %d, %s, %s\n",
...
AVCodec *getCodec(AVFormatContext *avtx, AVCodecContext *c)
{
	int i;
	AVCodec *codec = NULL;

	if (avtx->nb_streams == 0)
	{
		//return &flv_decoder;
		return NULL;
	}

	for (i=0; i<avtx->nb_streams; i++)
	{
		if (avtx->streams[i]->codec->codec_type == CODEC_TYPE_VIDEO)
		{
			c->extradata_size = avtx->streams[i]->codec->extradata_size;
			c->extradata = avtx->streams[i]->codec->extradata;

			for (codec = first_avcodec; codec != NULL; codec = codec->next)
			{
				if (codec->id ==avtx->streams[i]->codec->codec_id)
					return codec;
			}
		}
	}
