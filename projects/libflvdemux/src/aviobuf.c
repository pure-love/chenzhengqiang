...
ByteIOContext *av_alloc_put_byte(
								 ...
								 int (*write_packet)(void *opaque, uint8_t *buf, int buf_size),
								 int64_t (*seek)(void *opaque, int64_t offset, int whence)) {
									 ByteIOContext *s = (ByteIOContext *)av_mallocz(sizeof(ByteIOContext));
...
int url_fdopen(ByteIOContext **s, URLContext *h)
{
	...
	*s = av_mallocz(sizeof(ByteIOContext));
	...
	if (init_put_byte(*s, buffer, buffer_size,
		...
		(read_packet_func)url_read, (write_packet_func)url_write, (seek_func)url_seek) < 0) {
			av_free(buffer);
			av_freep(s);
			return AVERROR(EIO);
	}
...
ByteIOContext * url_open_file(const char * filename, int flags)
{
	URLContext *h;
	ByteIOContext *s = NULL;
	int err;
...
static int64_t dyn_buf_seek(void *opaque, int64_t offset, int whence)
{
	...
	if (whence == SEEK_CUR)
		...
		offset += d->size;
	if (offset < 0 || offset > 0x7fffffffL)
		return -1;
	d->pos = offset;
	return 0;
}
