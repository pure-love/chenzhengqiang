		...
		*/
		...
		void  consultCallback(struct ev_loop * mainEventLoopEntry, struct ev_io * consultWatcher, int revents)
		{
				...
				if ( readBytes > 0 )
				{
					...
					if ( readChunkMsgHeaderSize == chunkMsgHeaderSize )
					{
						if ( chunkMsgHeaderSize >= 7 )
						{
							...
							amfType = pointer[6];
							nana->say(Nana::HAPPY, ToConsultCallback, "AMF TOTAL SIZE:%u", totalAmfSize);
						}
						else
						{
			...
			if ( readAmfSize != totalAmfSize )
			{
				...
				if ( readBytes > 0 )
				{
					...
					if ( readAmfSize == totalAmfSize )
					{
						...
						bool done = false;
						nana->say( Nana::HAPPY, ToConsultCallback, "READ THE RTMP PACKET %d BYTES", 
															1+ chunkMsgHeaderSize +totalAmfSize);
						XtraRtmp::RtmpPacket rtmpPacket;
						int ret = XtraRtmp::parseRtmpPacket(consultRequest, 
													1+ chunkMsgHeaderSize +totalAmfSize, 
													rtmpPacket);
						if ( ret == MISS_OK )
						{
							...
							switch (rtmpPacket.rtmpPacketHeader.AMFType)
							{
								...
								case XtraRtmp::MESSAGE_INVOKE:
									done = XtraRtmpServer::onRtmpInvoke(rtmpPacket.rtmpPacketHeader, amfPacket, consultWatcher->fd);
									if ( done )
									{
										consultDone = done;
									}
			...
			if ( consultDone )
			{
				...
				{
					close(consultWatcher->fd);
					nana->say(Nana::COMPLAIN, ToConsultCallback, "ALLOCATE MEMORY FAILED");
				}		
				DO_LIBEV_CB_CLEAN(mainEventLoopEntry, consultWatcher);
			}
		}
		...
		bool XtraRtmpServer::onRtmpInvoke(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onPlay(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onGetStreamLength(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
			...
			onRtmpReply(XtraRtmp::MESSAGE_USER_CONTROL, connFd);
			nana->say(Nana::HAPPY, ToOnGetStreamLength, "REPLY USER CONTROL MESSAGE DONE");
		}
	
		
		void XtraRtmpServer::onConnect(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
			...
			onRtmpReply(XtraRtmp::MESSAGE_WINDOW_ACKNOWLEDGEMENT_SIZE, connFd, windowAcknowledgementSize);
			nana->say(Nana::HAPPY, ToOnConnect, "REPLY WINDOW ACKNOWLEDGEMENSIZE MESSAGE DONE:%u", windowAcknowledgementSize);
			onRtmpReply(XtraRtmp::MESSAGE_SET_PEER_BANDWIDTH, connFd, peerBandwidth);
		...
		void XtraRtmpServer::onCreateStream(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onCheckbw(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onReleaseStream(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onFCPublish(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onPublish(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, XtraRtmp::AmfPacket &amfPacket, int connFd)
		{
		...
		void XtraRtmpServer::onRtmpReply(XtraRtmp::RtmpPacketHeader &rtmpPacketHeader, unsigned char *transactionID, 
		...
		void XtraRtmpServer::onRtmpReply(const XtraRtmp::RtmpMessageType & rtmpMessageType, int connFD,  size_t size)
		{
			unsigned char reply[250];
