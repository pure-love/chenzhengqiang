...
namespace czq
{
	namespace service
	{
		...
		static const int DEFAULT_WORKTHREADS_SIZE = 8;
			...
			if ( listenFd_ >= 0 )
			{
				...
				else
				{
					//startup the threads pool,each thread attach a event loop
	    				...
	    				int totalThreads = atoi(serverConfig_.server["threads"].c_str());
					if ( totalThreads <= 0 )
		...
		*/
		...
		#define ToShakeHandCallback __func__
				...
				ssize_t readBytes = readv(receiveRequestWatcher->fd, iovRead, 2);
					...
					nana->say(Nana::HAPPY, ToShakeHandCallback, "C0 VERSION:%d C1.timestamp:%u", c0, ntohl(c1.timestamp));	
						s0=c0;
					XtraRtmpServer::S1 s1;
					XtraRtmpServer::S2 s2;
						s1.timestamp = htonl(0);
						s1.zeroOrTime = htonl(0);
						ServerUtil::generateSimpleRandomValue(s1.randomValue, XtraRtmpServer::RANDOM_VALUE_SIZE);
					s2.timestamp = c1.timestamp;
					s2.zeroOrTime = s1.timestamp;
					memcpy(s2.randomValue, c1.randomValue, XtraRtmpServer::RANDOM_VALUE_SIZE);	
		...
		#define ToParseRtmpAMF0 "parseRtmpAMF0"
				...
				while ( pos < len )
				{
					if ( !AMFObjectAlreadyStart )
					{
						switch (buffer[pos])
						{
							case XtraRtmpServer::TYPE_CORE_NUMBER:
								if ( countNumber == 0 )
								{
									countNumber += 1;
								...
								pos+=9;
								break;
							case XtraRtmpServer::TYPE_CORE_BOOLEAN:
								amfPacket.flag[0] = 1;
								amfPacket.flag[1] = (char)buffer[pos+1];
								pos+=2;
								break;
							case XtraRtmpServer::TYPE_CORE_STRING:
								...
								pos += stringLen;
								break;
							case XtraRtmpServer::TYPE_CORE_OBJECT:
								AMFObjectAlreadyStart = true;
								pos+=1; 
								break;
							case XtraRtmpServer::TYPE_CORE_NULL:
								pos+=1;
								break;
							case XtraRtmpServer::TYPE_CORE_MAP:
								pos+=5;
								break;	
						}
					}
					else
					{
						...
						pos+=keyLen;
						if (buffer[pos] == XtraRtmpServer::TYPE_CORE_STRING )
						{
							stringLen = buffer[pos+1]*16+buffer[pos+2];
							pos+=3;
			...
			void XtraRtmpServer::rtmpAMF0Dump(const AmfPacket & amfPacket)
			{
		...
		#define ToConsultCallback __func__
				...
				if ( chunkMsgHeaderSize >= 7 )
				{
					...
					pointer +=AMFSize-1;
					if ( AMFType == XtraRtmpServer::MESSAGE_INVOKE && format == 0 && channelID == 3)
					{
				...
				std::vector<XtraRtmpServer::RtmpPacket> rtmpPacketPool;
				...
				if ( ret == OK )
				{
					...
					while ( rpIter != rtmpPacketPool.end() )
					{
						...
						XtraRtmpServer::parseRtmpAMF0(rpIter->rtmpPacketPayload, rpIter->rtmpPacketHeader.AMFSize, 
												 amfPacket, (XtraRtmpServer::RtmpMessageType)rpIter->rtmpPacketHeader.AMFType);
						XtraRtmpServer::rtmpAMF0Dump(amfPacket);
						XtraRtmpServer::rtmpMessageDump((XtraRtmpServer::RtmpMessageType)rpIter->rtmpPacketHeader.AMFType);
						//the state machine
						switch (rpIter->rtmpPacketHeader.AMFType)
						{
							case XtraRtmpServer::MESSAGE_CHANGE_CHUNK_SIZE:
								break;
							case XtraRtmpServer::MESSAGE_DROP_CHUNK:
								break;
							case XtraRtmpServer::MESSAGE_SEND_BOTH_READ:
								break;
							case XtraRtmpServer::MESSAGE_USER_CONTROL:
								nana->say(Nana::HAPPY, ToConsultCallback, "RECEIVE THE MESSAGE OF USER CONTROL");
								break;
							case XtraRtmpServer::MESSAGE_WINDOW_ACKNOWLEDGEMENT_SIZE:
								nana->say(Nana::HAPPY, ToConsultCallback, "RECEIVE THE MESSAGE OF WINDOW ACKNOWLEDGEMENT SIZE");
								break;
							case XtraRtmpServer::MESSAGE_SET_PEER_BANDWIDTH:
								break;
							case XtraRtmpServer::MESSAGE_AUDIO:
								break;
							case XtraRtmpServer::MESSAGE_VIDEO:
								break;
							case XtraRtmpServer::MESSAGE_AMF0_DATA:
								break;
							case XtraRtmpServer::MESSAGE_SUBTYPE:
								break;
							case XtraRtmpServer::MESSAGE_INVOKE:
								done = XtraRtmpServer::onRtmpInvoke(rpIter->rtmpPacketHeader, amfPacket, consultWatcher->fd);
								if (done)
								{
									consultDone = done;
								}
			...
			void XtraRtmpServer::rtmpMessageDump(const RtmpMessageType & rtmpMessageType)
			{
		...
		#define ToReceiveStreamCallback __func__
				...
				if ( chunkMsgHeaderSize >= 7 )
				{
					...
					if ( msgType ==  XtraRtmpServer::MESSAGE_VIDEO || msgType == XtraRtmpServer::MESSAGE_AUDIO )
					{
						if ( AMFSize > 128 )
						{
						...
						if ( msgType ==  XtraRtmpServer::MESSAGE_VIDEO )
						{
					...
					XtraRtmpServer::rtmpMessageDump((XtraRtmpServer::RtmpMessageType)msgType);
					...
					if ( msgType == XtraRtmpServer::MESSAGE_AMF0_DATA )
					{
						totalBytes = 0;
			...
			bool XtraRtmpServer::onRtmpInvoke(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define onRtmpInvoke __func__
			...
			void XtraRtmpServer::onPlay(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		...
		#define ToOnPlay __func__
			...
			void XtraRtmpServer::onGetStreamLength(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToOnGetStreamLength __func__
			...
			void XtraRtmpServer::onConnect(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToOnConnect __func__
			...
			void XtraRtmpServer::onCreateStream(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToOnCreateStream __func__
			...
			void XtraRtmpServer::onCheckbw(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToOnCheckbw __func__
			...
			void XtraRtmpServer::onReleaseStream(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToReleaseStream __func__
			...
			void XtraRtmpServer::onFCPublish(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToFCPublish __func__
			...
			void XtraRtmpServer::onPublish(RtmpPacketHeader &rtmpPacketHeader, AmfPacket &amfPacket, int connFd)
			{
		#define ToOnPublish __func__
			...
			void XtraRtmpServer::onRtmpReply(RtmpPacketHeader &rtmpPacketHeader, unsigned char *transactionID, 
		...
		#define ToOnRtmpReply __func__
				...
				rtmpPack[7] = XtraRtmpServer::MESSAGE_INVOKE;
													  ...
													  const char * parameters[][2], int rows)
			{
				...
				int row = 0;
				reply[row]=XtraRtmpServer::TYPE_CORE_STRING;
				size_t AMFSize = 1;
				...
				reply[AMFSize]=XtraRtmpServer::TYPE_CORE_NUMBER;
				AMFSize += 1;
				if (transactionID != 0)
				{
				...
				for(row=1; row < rows; ++row)
				{
					...
					if (parameters[row][0] == 0 && parameters[row][1] == 0)
					{
						reply[AMFSize]=XtraRtmpServer::TYPE_CORE_NULL;
						AMFSize += 1;
					}
					else 
					{
				...
				if ( object )
				{
					...
					reply[AMFSize]=XtraRtmpServer::TYPE_CORE_OBJECT;
					AMFSize += 1;
				}
				...
				for (row=1; row < rows; ++row)
				{
					...
					AMFSize += len;
					reply[AMFSize]=XtraRtmpServer::TYPE_CORE_STRING;
					AMFSize += 1;
			...
			void XtraRtmpServer::onRtmpReply(const RtmpMessageType & rtmpMessageType, int connFD,  size_t size)
			{
				unsigned char reply[250];
				...
				switch (rtmpMessageType)
				{
					case XtraRtmpServer::MESSAGE_WINDOW_ACKNOWLEDGEMENT_SIZE:
						...
						pos+=3;
						reply[pos]=XtraRtmpServer::MESSAGE_WINDOW_ACKNOWLEDGEMENT_SIZE;
						pos += 1;
						pos += 4;
					...
					case XtraRtmpServer::MESSAGE_SET_PEER_BANDWIDTH:
						...
						reply[pos+2] = static_cast<unsigned char>(AMFSize);
						pos+=3;
						reply[pos]=XtraRtmpServer::MESSAGE_SET_PEER_BANDWIDTH;
						pos += 1;
						pos += 4;
						reply[pos] = static_cast<unsigned char>((size & 0xff000000) >> 24);
						reply[pos+1] = static_cast<unsigned char>((size & 0x00ff0000) >> 16);
					...
					case XtraRtmpServer::MESSAGE_USER_CONTROL:
						...
						reply[pos+2] = static_cast<unsigned char>(AMFSize);
						pos+=3;
						reply[pos]=XtraRtmpServer::MESSAGE_USER_CONTROL;
						pos += 1;
						pos += 4;
						pos += 6;
						NetUtil::writeSpecifySize2(connFD, reply, pos);
